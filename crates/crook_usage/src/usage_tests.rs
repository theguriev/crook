...
/// The wire tests above pin a captured response; this one is the only check
/// that the endpoint still speaks that shape. It needs the network and a real
/// Claude Code session, so it stays out of the default run.
#[test]
#[ignore = "hits the live usage endpoint with this machine's Claude Code session"]
fn test_reads_live_usage_from_the_local_session() {
    let token = crate::credentials::load_access_token().expect("a local Claude Code session");
    let snapshot = fetch_usage(&token.token).expect("the usage endpoint should answer");

    assert!(
        (0. ..=100.).contains(&snapshot.session_percent),
        "session percent out of range: {snapshot:?}"
    );
}
