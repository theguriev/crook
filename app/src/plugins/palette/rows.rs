...
//! # Two lists, one box
//!
//! The palette is one surface with two lists in it, and which one it is
//! showing is worked out from the query rather than held in a flag — see
//! [`Mode`]. Run mode is today's launcher, flat and title-sorted, and it comes
//! out of this module unchanged: a list of nothing but [`Row::Command`] is a
//! list with no headings in it, and every method below degrades to the
...
use crate::plugins::shortcuts::{NOT_BOUND, PANE_KEYS, PaneKey};
use crate::workspace::Workspace;

use super::state::Palette;
...
/// Which of the two lists the query asks for, and what is left to search with.
///
...
pub(super) enum Mode {
    /// Everything there is to run, flat and by title. Today's launcher.
    Run,
...
impl Mode {
    /// VSCode's own help sigil, and the one character no command's title
    /// starts with.
    pub(super) const SIGIL: char = '?';

    /// Which list `query` asks for, and what of it is left to search with.
    pub(super) fn of(query: &str) -> (Self, &str) {
        match query.strip_prefix(Self::SIGIL) {
            Some(rest) => (Self::Keys, rest),
            None => (Self::Run, query),
        }
    }
}
...
impl Rows {
    ...
    pub(super) fn command_at(&self, at: usize) -> Option<(&Entry, ActionId)> {
        match self.rows.get(at) {
            Some(Row::Command(entry, id)) => Some((entry, *id)),
            _ => None,
        }
    }
...
///
/// Worked out every frame rather than cached, because the three things it
/// depends on — the query, which plugins are loaded, and what the keybindings
/// say — all change without this plugin being told.
///
...
pub(super) fn showing(workspace: &Workspace, palette: &Palette) -> Rows {
    ...
    let mut matched: Vec<(&PluginId, Entry, ActionId)> = Vec::new();
    ...
    let rows = match mode {
        Mode::Run => flat(matched),
        Mode::Keys => {
            let mut rows = grouped(host, &sizes, matched);
...
/// writing a pattern. The chord text is in the haystack in **both** modes,
/// because typing `cmd+k` and being told it searches the tabs is a win on the
/// fast path too — and it is the half of "what is bound to what" that a list
/// of titles cannot answer.
fn command_matches(
    title: &str,
    action: &ActionName,
    owner: &PluginId,
...
/// Run mode: no headings, and today's ordering exactly.
///
...
fn flat(mut matched: Vec<(&PluginId, Entry, ActionId)>) -> Vec<Row> {
    matched.sort_by(|(_, left, _), (_, right, _)| left.title.cmp(&right.title));
    matched
        .into_iter()
        .map(|(_, entry, id)| Row::Command(entry, id))
        .collect()
}
...
fn grouped(
    ...
    matched: Vec<(&PluginId, Entry, ActionId)>,
) -> Vec<Row> {
    let mut owners: Vec<&PluginId> = Vec::new();
    let mut buckets: Vec<Vec<(Entry, ActionId)>> = Vec::new();
    let mut elsewhere: Vec<(Entry, ActionId)> = Vec::new();

    for (owner, entry, id) in matched {
        if sizes.get(owner).copied().unwrap_or_default() < GROUP_MINIMUM {
            elsewhere.push((entry, id));
            continue;
        }
        match owners.iter().position(|known| *known == owner) {
            Some(at) => buckets[at].push((entry, id)),
            None => {
                owners.push(owner);
                buckets.push(vec![(entry, id)]);
            }
        }
    }
...
fn group(name: String, mut entries: Vec<(Entry, ActionId)>) -> Vec<Row> {
    if entries.is_empty() {
        return Vec::new();
    }
    ...
    rows.extend(
        entries
            .into_iter()
            .map(|(entry, id)| Row::Command(entry, id)),
    );
    rows
}
